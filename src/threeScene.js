...
const light = new THREE.PointLight(0xffffff, 121, 100);
...
const loadMaze = (maze) => {
  ...
  loader.load(
    ...
    function (geometry) {
      ...
      mazeMesh.material.color.setHex(colors[0]);
...
function animate() {
  ...
  controls.update();
  updateMesh(mazeMesh, elapsedTime);
  render();
  stats.update();
}
...
export const changeMesh = (x, y, z) => {
  if (mazeMesh.material.color.r === 1) {
    mazeMesh.material.color.setHex(colors[1]);
  } else {
    mazeMesh.material.color.setHex(colors[0]);
  }
  render();
